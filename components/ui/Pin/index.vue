...
defineProps<PinContainerProps>()

const transform = ref('translate(-50%,-50%) rotateX(0deg)')
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <div class="relative z-50" :class="className">
          <slot />
        </div>
      </div>
    </div>
    <ClientOnly>
      <UiPinPerspective :title="title" :href="href" />
    </ClientOnly>
  </div>
</template>
